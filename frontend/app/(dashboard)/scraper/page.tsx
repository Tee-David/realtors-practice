    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 md:px-8 py-8 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-3 relative z-10">
          <div className="hidden md:flex items-center gap-2 px-4 py-2 rounded-2xl bg-background/60 backdrop-blur-xl border border-white/10 shadow-sm text-sm font-medium">
            <div className="relative flex items-center justify-center w-3 h-3">
              <span className={`absolute inline-flex h-full w-full rounded-full opacity-75 ${isConnected ? "bg-green-500 animate-ping duration-1000" : "bg-amber-400"}`} />
              <span className={`relative inline-flex rounded-full h-2 w-2 ${isConnected ? "bg-green-500" : "bg-amber-400"}`} />
            </div>
            <span className="text-muted-foreground">
              {isConnected ? "Engine Connected" : "Engine Standby"}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
        ...
        <div className="lg:col-span-8 flex flex-col h-full min-h-[500px] lg:min-h-0">
          <Card className="bg-white dark:bg-[#0A0A0B] border border-border dark:border-white/10 shadow-2xl relative overflow-hidden flex-1 flex flex-col rounded-3xl group">
            <div className={`absolute -top-40 -right-40 w-96 h-96 rounded-full blur-[120px] pointer-events-none transition-colors duration-1000 ${activeJob ? "bg-primary/10 dark:bg-primary/20" : "bg-secondary/30 dark:bg-zinc-800/20"}`} />
            <CardHeader className="pb-3 border-b border-border dark:border-white/10 bg-slate-50/80 dark:bg-[#0A0A0B]/80 backdrop-blur-xl z-10 flex-shrink-0 relative">
              <div className="flex items-center justify-between">
            ...
            <CardContent className="p-5 flex-1 overflow-y-auto font-mono text-xs sm:text-sm bg-white dark:bg-[#0A0A0B] text-slate-700 dark:text-zinc-300 relative z-0 scroller">
      ...
      {/* Operation Errors Section */}
      {jobs && jobs.some(j => j.status === 'FAILED' && (j.errorMessage || j.failedItems > 0)) && (
        <div className="mt-6">
          <h2 className="text-sm font-semibold flex items-center gap-2 text-red-500 mb-3">
            <AlertCircle className="w-4 h-4" />
            Recent Operation Errors
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {jobs.filter(j => j.status === 'FAILED').slice(0, 6).map(job => (
              <Card key={job.id} className="bg-red-500/5 border border-red-500/15 shadow-sm relative overflow-hidden">
                ...
                <CardContent className="p-4 pl-5 flex flex-col gap-2">
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <span className="text-[10px] uppercase font-bold tracking-widest text-red-400 bg-red-500/10 px-2 py-0.5 rounded">FAILED</span>
                  </div>
                  <p className="text-sm text-foreground/90 font-mono leading-relaxed break-words mt-1">
                    {job.errorMessage || `${job.failedItems} item(s) failed during extraction`}
                  </p>
                  <div className="text-[11px] text-muted-foreground mt-1 flex justify-between border-t border-white/5 pt-2">
                    <span>Indexed: {job.successfulItems}</span>
